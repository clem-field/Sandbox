...
# Format mitigations as readable string
def format_mitigations(mitigations):
    ...
    for mit in mitigations:
        for item in mit:
            phase = ', '.join(item.get('Phase', []))
            desc = item.get('Description', '')
            formatted.append(f"Phase: {phase} \n Description: {desc}")
    return ' \n '.join(formatted)

# Format applicable platforms as readable string
def format_applicable_platforms(platforms):
    if not platforms:
        return "No applicable platforms."
    formatted = []
    for plat_list in platforms:
        for plat in plat_list:
            formatted.append(f"Type: {plat.get('Type', '')}, Class: {plat.get('Class', '')}, Prevalence: {plat.get('Prevalence', '')}")
    return ' \n '.join(formatted)
...
def generate_ruby_control(cwe_id, description, aggregated_locations, applicable_platforms, potential_mitigations, severity, nist_controls, grc_controls, owasp_ids, category, org_refs, nist_references, related_controls):
    ...
    applicable_platforms_str = format_applicable_platforms(lib.json.loads(applicable_platforms) if applicable_platforms else [])
    potential_mitigations_str = format_mitigations(lib.json.loads(potential_mitigations) if potential_mitigations else [])
...
def convert_to_hdf(sast_report, cwe_data, catalog_data, thresholds):
    ...
    for unique_cwe_id, matching_vulns in sorted(cwe_to_vulns.items()):
        ...
        mapped_identifiers = [{"url": url} for url in sorted(unique_urls)]  # Unique refs
        ...
        applicable_platforms = format_applicable_platforms(cwe_entry.get('applicable_platforms', [])) if cwe_entry else ''
        potential_mitigations = format_mitigations(cwe_entry.get('potential_mitigations', [])) if cwe_entry else ', '.join(set(v.get('solution', '') for v in matching_vulns if v.get('solution', '')))
        ...
        save_ruby_control(unique_cwe_id, ruby_content)
