...
#!/usr/bin/env python3
"""
Merge cas_enriched.json + cas_schema.json → cas_final_schema.json
Preserves metadata, generates TODO_MAPPING.md, and gives clear console feedback.
"""
...
def merge_rules(enriched_data: Dict, schema_rule_map: Dict[str, Dict]) -> tuple[List[Dict], List[str]]:
    ...
    for enriched_rule in enriched_rules:
        ...
        if not rule_name:
            print(f"Warning: Skipping enriched rule without 'rule_name'")
            continue
        ...
        if rule_name in schema_rule_map:
            ...
            # If rule has schema data but no NIST controls → still needs mapping
            has_controls = (
    ...
    # Add rules that exist only in schema
...
def write_todo_markdown(needs_mapping: List[str], output_path: Path):
    ...
    md_content = "# Rules Requiring Mapping / Review\n\n"
    md_content += "These rules are missing either:\n"
    md_content += "- Schema metadata (severity, description, resources), **or**\n"
    md_content += "- NIST / secondary control mappings\n\n"

    for rule in sorted(needs_mapping):
        md_content += f"- [ ] `{rule}`\n"

    md_content += f"\n**Total**: {len(needs_mapping)} rule(s) need attention\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Merge CAS enriched + schema files and generate mapping todo list"
    )
    ...
    print(f"   • {len(enriched_data.get('rules', [])):>3} rules from enriched file")
    print(f"   • {len(schema_rule_map):>3} rules from schema file")

    merged_rules, needs_mapping = merge_rules(enriched_data, schema_rule_map)

    # Preserve original top-level structure from enriched file
    ...
    print(f"\nSuccess: Merged and wrote {len(merged_rules)} rules → {output_path.name}")
    print("   Preserved metadata, schema_info, usage_guidelines, change_log from input")

    if needs_mapping:
        print("\n" + "═" * 70)
        print("WARNING: The following rules lack complete information")
        print("         (missing schema details OR NIST/control mappings)")
        print("═" * 70)
        for rule in sorted(needs_mapping):
            print(f"   • {rule}")
        print(f"\n   Total incomplete: {len(needs_mapping)} rule(s)")
        print("   These have been included with placeholders and empty control arrays.\n")

        write_todo_markdown(needs_mapping, output_path)
        print("   → A GitHub-style checklist has been saved to TODO_MAPPING.md")
    else:
        print("\nAll rules are fully matched and enriched! No action needed.")
